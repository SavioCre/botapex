...
async def on_ready():
    ...
    await client.change_presence(status = discord.Status.online, activity = discord.Game(name = "Dando surra de pau mole no savioc"))
...
@client.command(pass_context=True)
async def savioc(ctx,verbo="é",adjetivo=None):
    if channelName in ctx.message.channel.name or "level" in ctx.message.channel.name:
        if verbo == "é":
            if adjetivo is not None and adjetivo == 'gay?':
                if ctx.author.id == 611804075528421388:
                    await ctx.channel.send("Não, tu que é gay!")
                elif ctx.author.id == 183631176190132225:
                    await ctx.channel.send("Sim, mas ainda é bem menos que o Jhonny!")
                else:
                    await ctx.channel.send("Sim!")
            else:
                if ctx.author.id == 212628979998261248:
                    await ctx.channel.send("Pode continuar carregando eles {0.mention}".format(ctx.message.author))
                else:
                    await ctx.channel.send("Cala a boca e deixa o Peluti te carregar")
    else:
